...
describe("BattleSystem", function()
    local function createBattle(units)
    ...
    local function setupBattle()
        return createBattle({
            Unit.new({ id = "ally", faction = "allies", col = 2, row = 2, move = 3, attackPower = 45 }),
            Unit.new({ id = "enemy", faction = "enemies", col = 4, row = 2, hp = 40 })
        })
    end

    local function setupBattleWithBlocker()
        return createBattle({
    ...
    describe("movement range", function()
        it("prevents moving through occupied tiles", function()
            local context = setupBattleWithBlocker()
    ...
    describe("attacks", function()
        it("applies damage and removes defeated targets", function()
            ...
            battleSystem:move(attacker, 3, 2)
