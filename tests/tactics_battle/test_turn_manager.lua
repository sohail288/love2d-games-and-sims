...
describe("TurnManager", function()
    ...
    it("provides a snapshot of the current turn order", function()
        local alpha = Unit.new({ id = "alpha", speed = 12 })
        local beta = Unit.new({ id = "beta", speed = 9 })
        local gamma = Unit.new({ id = "gamma", speed = 6 })
        local manager = TurnManager.new({ gamma, alpha, beta })
        local order = manager:getTurnOrder()
        assertEquals(order[1].id, "alpha")
        assertEquals(order[2].id, "beta")
        assertEquals(order[3].id, "gamma")
        assertEquals(manager:getCurrentIndex(), 1)
    end)
end)
