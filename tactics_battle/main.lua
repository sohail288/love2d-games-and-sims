...
local function refreshTurnQueue()
    if not state.turnManager then
        state.turnQueue = {}
        return
    end
    state.turnQueue = state.turnManager:getOrder()
end
...
local function drawHud()
    ...
    if state.turnQueue and #state.turnQueue > 0 then
        local labels = {}
        local current = state.turnManager:currentUnit()
        for _, unit in ipairs(state.turnQueue) do
            local name = unit.name
            if current and current.id == unit.id then
...
local function handleCurrentTurn(unit)
    if not unit or state.battleOutcome then
        return
    end
...
function love.load()
    ...
    state.enemyAI = EnemyAI.new(state.battleSystem)
    local current = state.turnManager:currentUnit()
